...
Kode simulator berada di `assistant/`. Endpoint diagnostik webhook WhatsApp Cloud API tetap tersedia di `/api/whatsapp`.

Bot operasional WhatsApp Web berada di `assistant/bot/` dan dijalankan sebagai proses Node.js terpisah:
...
Konfigurasi DeepSeek, Google Sheets, Google Drive, session QR, takeover admin 24 jam, dan auto-start Windows dijelaskan di `docs/whatsapp-web-bot-operations.md`.
